...
# Free-tier project limit is enforced at 3 projects.
class ProjectLimitExceeded(ValueError):
    """Raised when a Free-plan user tries to add more than FREE_LIMITS['max_projects']."""

    def __init__(self, current_count: int, limit: int):
        self.current_count = current_count
        self.limit = limit
        super().__init__(
            f"Free plan supports up to {limit} watched projects "
...
class ProjectRegistry:
    ...
    def add(self, path: str, name: Optional[str] = None,
        ...
        from core.billing import FREE_LIMITS, Feature, feature_enabled
        if not feature_enabled(Feature.UNLIMITED_PROJECTS):
            limit = FREE_LIMITS.get("max_projects", 3)
            current_count = len(self.list())
            if current_count >= limit:
                raise ProjectLimitExceeded(current_count=current_count, limit=limit)
